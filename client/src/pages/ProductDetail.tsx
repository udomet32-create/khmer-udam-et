...
import { ArrowLeft, ArrowRight, Maximize2, Minus, Play, Plus, Send, ShoppingBag, Video, X } from "lucide-react";
...
export default function ProductDetail() {
  ...
  const productQuery = trpc.products.getById.useQuery({ id }, { enabled: Number.isInteger(id) && id > 0, retry: false, staleTime: 5 * 60 * 1000 });
  ...
  const toggleFullscreen = async () => {
    ...
    const target = videoRef.current || mediaRef.current;
    if (!target) return;
    const mobileVideo = target as HTMLVideoElement & { webkitEnterFullscreen?: () => void };
    if (typeof mobileVideo.webkitEnterFullscreen === "function") { mobileVideo.webkitEnterFullscreen(); return; }
