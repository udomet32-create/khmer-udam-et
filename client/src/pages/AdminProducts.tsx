...
const emptyForm: ProductForm = { name: "", code: "", category: "កាបូប", description: "", price: "", oldPrice: "", badge: "", imageUrl: "", imageUrls: [], videoUrl: "", colors: "", sizes: "", sortOrder: "0", isActive: true };
const categories = ["កាបូប", "ស្បែកជើង", "ខ្សែក្រវ៉ាត់", "ម៉េកអាប់"];

function toForm(product: AdminProduct): ProductForm {
  return { name: product.name, code: product.code, category: product.category, description: product.description ?? "", price: (product.priceCents / 100).toFixed(2), oldPrice: product.oldPriceCents ? (product.oldPriceCents / 100).toFixed(2) : "", badge: product.badge ?? "", imageUrl: product.imageUrl ?? "", imageUrls: product.imageUrls ? (() => { try { return JSON.parse(product.imageUrls); } catch { return product.imageUrl ? [product.imageUrl] : []; } })() : (product.imageUrl ? [product.imageUrl] : []), videoUrl: product.videoUrl ?? "", colors: product.colorOptions ? (() => { try { return JSON.parse(product.colorOptions).join(", "); } catch { return ""; } })() : "", sizes: product.sizeOptions ? (() => { try { return JSON.parse(product.sizeOptions).join(", "); } catch { return ""; } })() : "", sortOrder: String(product.sortOrder), isActive: product.isActive };
}
...
export default function AdminProducts() {
  ...
  const saveProduct = async (event: React.FormEvent) => {
    ...
    setMessage("");
    const payload = { name: form.name.trim(), code: form.code.trim(), category: form.category, description: form.description.trim() || null, priceCents: Math.round(Number(form.price) * 100), oldPriceCents: form.oldPrice ? Math.round(Number(form.oldPrice) * 100) : null, imageUrl: form.imageUrls[0] || form.imageUrl || null, imageUrls: JSON.stringify(form.imageUrls.slice(0, 9)), videoUrl: form.videoUrl || null, colorOptions: JSON.stringify(form.colors.split(",").map((value) => value.trim()).filter(Boolean)), sizeOptions: JSON.stringify(form.sizes.split(",").map((value) => value.trim()).filter(Boolean)), badge: form.badge.trim() || null, sortOrder: Number(form.sortOrder) || 0, isActive: form.isActive };
  ...
  const uploadMedia = async (file: File, kind: "image" | "video") => {
    ...
    try {
      ...
      if (kind === "image") { setForm((current) => { const imageUrls = [...current.imageUrls, result.url].slice(0, 9); return { ...current, imageUrl: current.imageUrl || result.url, imageUrls }; }); setMessage("រូបភាព upload ជោគជ័យ"); } else { setField("videoUrl", result.url); setMessage("វីដេអូ upload ជោគជ័យ"); }
