...
import { Facebook, Heart, Instagram, MapPin, Menu, MessageCircle, Phone, Search, Send, Share2, ShoppingBag, X } from "lucide-react";
...
export default function Home() {
  ...
  return <div className="storefront">
    ...
    {cartOpen && <div className="drawer-backdrop" onClick={() => setCartOpen(false)}><aside className="cart-drawer" onClick={(e) => e.stopPropagation()}><div className="drawer-header"><div><p className="section-kicker">YOUR ORDER</p><h2>កន្ត្រករបស់អ្នក ({cart.length})</h2></div><button className="close-button" onClick={() => setCartOpen(false)}><X size={20} /></button></div>{cartProducts.length === 0 ? <div className="cart-empty"><ShoppingBag size={32} /><h3>កន្ត្រកនៅទទេ</h3><p>បន្ថែមទំនិញដែលអ្នកចូលចិត្ត ដើម្បីចាប់ផ្តើម order។</p></div> : <><div className="cart-items">{cartProducts.map((p) => <div className="cart-item" key={p.id}><div className="mini-product"><ProductVisual product={p} /></div><div className="cart-item-copy"><p>{p.category}</p><h3>{p.khmerName}</h3><strong>{price(p.price)}</strong></div><button className="remove-item" onClick={() => toggleCart(p.id)}><X size={15} /></button></div>)}</div><div className="cart-summary"><div><span>សរុប</span><strong>{price(total)}</strong></div><a className="primary-button full" href="https://t.me/" target="_blank" rel="noreferrer"><Send size={17} /> Order តាម Telegram</a></div></>}</aside></div>}
  </div>;
}
