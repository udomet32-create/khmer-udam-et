...
import { ArrowRight, CircleDot, Facebook, Footprints, Heart, Instagram, LayoutGrid, MapPin, Menu, MessageCircle, Minus, Phone, Plus, Search, Send, Share2, ShoppingBag, Sparkles, X } from "lucide-react";
...
const categoryIcons = { "ទាំងអស់": LayoutGrid, "កាបូប": ShoppingBag, "ស្បែកជើង": Footprints, "ខ្សែក្រវ៉ាត់": CircleDot, "ម៉េកអាប់": Sparkles } as const;
...
export default function Home() {
  ...
  return <div className="storefront">
    ...
    <main id="top">
      ...
      <section className="premium-connect-section">
        <button className={`premium-connect-trigger ${contactShareOpen ? "open" : ""}`} onClick={() => setContactShareOpen((value) => !value)} aria-expanded={contactShareOpen}>
          <span className="premium-connect-icon"><Share2 size={20} /></span><span><strong>ទំនាក់ទំនង និងចែករំលែក</strong><small>Connect & share Khmer Udam ET</small></span><span className="premium-connect-chevron">{contactShareOpen ? "−" : "+"}</span>
        </button>
        {contactShareOpen && <div className="premium-connect-panel">
          <div className="premium-contact-grid"><a href="https://t.me/oudom_online_shop" target="_blank" rel="noreferrer" className="premium-contact-item telegram"><Send size={18} /><span>Telegram</span></a><a href="https://www.facebook.com/share/19ajM7PjU8/?mibextid=wwXIfr" target="_blank" rel="noreferrer" className="premium-contact-item facebook"><Facebook size={18} /><span>Facebook</span></a><a href="tel:+855888717671" className="premium-contact-item phone"><Phone size={18} /><span>088 871 7671</span></a><a href="https://www.whatsapp.com/business/download/" target="_blank" rel="noreferrer" className="premium-contact-item whatsapp"><MessageCircle size={18} /><span>WhatsApp</span></a><a href="https://maps.app.goo.gl/EaQbHNijNE7EHgmFA?g_st=ic" target="_blank" rel="noreferrer" className="premium-contact-item location"><MapPin size={18} /><span>ទីតាំង</span></a></div>
          <div className="premium-share-heading"><Share2 size={16} /><span>ចែករំលែក</span></div><div className="share-buttons premium-share-buttons"><a href={facebookShareHref} target="_blank" rel="noreferrer"><Facebook size={15} /> Facebook</a><a href={telegramShareHref} target="_blank" rel="noreferrer"><Send size={15} /> Telegram</a><a href={whatsappShareHref} target="_blank" rel="noreferrer"><MessageCircle size={15} /> WhatsApp</a><button onClick={async () => { await navigator.clipboard?.writeText(`${shareText}\n${shareUrl}`); setShareCopied(true); window.setTimeout(() => setShareCopied(false), 2200); }}><Share2 size={15} /> {shareCopied ? "បានចម្លង ✓" : "ចម្លងតំណ"}</button></div>
        </div>}
      </section>

      <section id="collection" className="collection-section reference-collection"><div className="reference-categories">{categories.map((item) => { const CategoryIcon = categoryIcons[item as keyof typeof categoryIcons] || LayoutGrid; return <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)} aria-pressed={category === item}><CategoryIcon className="category-icon" size={16} strokeWidth={1.8} /><span>{item}</span><small>{item === "ទាំងអស់" ? catalog.length : catalog.filter((product) => product.category === item).length}</small></button>; })}</div><div className="search-box reference-search"><Search size={20} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ស្វែងរកលេខកូដ ឬឈ្មោះទំនិញ..." aria-label="ស្វែងរកតាមលេខកូដ ឬឈ្មោះទំនិញ" /></div><div className="product-grid">{filtered.map((product) => <article className="product-card" key={product.id}><div className="product-media" onClick={() => window.location.assign(`/product/${product.id}`)} role="link" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && window.location.assign(`/product/${product.id}`)}><ProductVisual product={product} />{product.badge && <span className="product-badge">{product.badge}</span>}<button className={`favorite-button ${favorites.includes(product.id) ? "liked" : ""}`} onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }}><Heart size={18} fill={favorites.includes(product.id) ? "currentColor" : "none"} /></button></div><div className="product-info"><p className="product-category">{product.category} · {product.code}</p><h3>{product.khmerName}</h3><div className="product-bottom"><div><strong>{price(product.price)}</strong>{product.oldPrice && <del>{price(product.oldPrice)}</del>}</div><button className={`add-button ${cart.includes(product.id) ? "added" : ""}`} onClick={() => toggleCart(product.id)}>{cart.includes(product.id) ? "បានបន្ថែម ✓" : "បន្ថែម"}</button></div></div></article>)}</div>{filtered.length === 0 && <div className="empty-state"><Search size={28} /><h3>រកមិនឃើញទំនិញ</h3><p>សូមសាកល្បลองលេខកូដ ឬឈ្មោះទំនិញផ្សេងទៀត។</p></div>}</section>
    </main>
